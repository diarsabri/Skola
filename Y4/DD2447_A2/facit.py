...
def estimate_position(alphabet, sequences, prev_positions, current_seq_idx, w, alpha_seq, alpha_motif):
    ...
    for r_i in range(M-w):
        ...
        backgrounds = [sequences[i][:positions[i]] + sequences[i][positions[i]+w:] for i in range(N)]
        ...
        motifs = [sequences[i][positions[i]:positions[i]+w] for i in range(N)]
        c_motif = [Counter([motifs[i][j] for i in range(N)]) for j in range(w)]

        background_base = float(math.gamma(sum(alpha_seq))) / math.gamma(K*(M-w) + sum(alpha_seq))
        motif_base = float(math.gamma(sum(alpha_motif))) / math.gamma(K + sum(alpha_motif))

        # Compute probabilities
        prod = [float(math.gamma(c_background[alphabet[k]] + alpha_seq[k])) / math.gamma(alpha_seq[k]) for k in range(K)]
        p_background = background_base * functools.reduce(operator.mul, prod, 1)

        for j in range(w):
            prod = [float(math.gamma(c_motif[j][alphabet[k]] + alpha_motif[k])) / math.gamma(alpha_motif[k]) for k in range(K)]
...
def estimate_starting_positions(alphabet, sequences, alpha_seq, alpha_motif, w,
    ...
    positions.append([random.randint(0, M-w+1) for i in range(N)])
    ...
    positions = [positions[j] for j in range(minstep, iterations, steplength)]

    # For each sequence, the selected positions are the ones the most elected during the previous iterations
    return [Counter([positions[j][i] for j in range(len(positions))]).most_common(1)[0][0] for i in range(N)]


if __name__ == '__main__':
    alphabet = [1,2,3,4]
    alpha_seq = [1,1,1,1]
    alpha_motif = [0.8,0.8,0.8,0.8]
